    ...
    <div class="p-4">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('nombre') is-invalid @enderror"
                wire:model="nombre" placeholder="Ej: Mi Tienda Principal" autofocus style="border-radius:10px;">
            @error('nombre') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        {{-- Telefono --}}
        <div class="mb-3">
            <label class="form-label fw-semibold text-muted">Telefono <span class="fw-normal">(Opcional)</span></label>
            <input type="text" class="form-control @error('telefono') is-invalid @enderror"
                wire:model="telefono" placeholder="Ej: 76543210" style="border-radius:10px;">
            @error('telefono') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        {{-- Direccion --}}
        <div class="mb-3">
            <label class="form-label fw-semibold text-muted">Direccion <span class="fw-normal">(Opcional)</span></label>
            <input type="text" class="form-control @error('direccion') is-invalid @enderror"
                wire:model="direccion" placeholder="Ej: Av. Principal #123" style="border-radius:10px;">
        ...
        <div class="mb-4">
            ...
            <div class="d-flex flex-wrap gap-2 mt-1">
                ...
                <button type="button"
                    ...
                    title="{{ $tema['nombre'] }}"
                    style="width:36px;height:36px;border-radius:50%;background:{{ $tema['color'] }};border:3px solid {{ $theme_number == $num ? '#333' : 'transparent' }};outline:2px solid {{ $theme_number == $num ? $tema['color'] : 'transparent' }};outline-offset:2px;cursor:pointer;transition:all .15s;">
                </button>
                @endforeach
            </div>
            <div class="mt-2 d-flex align-items-center gap-2" style="font-size:0.82rem;color:#666;">
                <span style="width:14px;height:14px;border-radius:50%;background:{{ $temas[$theme_number]['color'] ?? '#29adb2' }};display:inline-block;"></span>
                <span>{{ $temas[$theme_number]['nombre'] ?? 'Teal' }}</span>
            </div>
        </div>

        {{-- Botones --}}
        <div class="d-flex gap-2 mt-2">
            <a href="{{ route('ventas') }}" class="btn btn-outline-secondary flex-shrink-0" style="border-radius:10px;padding:10px 20px;">
